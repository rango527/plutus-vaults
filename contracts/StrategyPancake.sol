...
contract StrategyPancake is Ownable, ReentrancyGuard, Pausable {
    ...
    uint256 public controllerFee = 220;
    ...
    uint256 public rewardPoolRate = 50;
    uint256 public constant rewardPoolRateMax = 10000; // 100 = 1%
    uint256 public constant rewardPoolRateUL = 500;
    ...
    function distributeFees(uint256 _earnedAmt) internal returns (uint256) {
        if (_earnedAmt > 0) {
            ...
            if (rewardPoolRate > 0) {
                uint256 fee =
                    _earnedAmt.mul(rewardPoolRate).div(rewardPoolRateMax);
                IBEP20(earnedAddress).safeTransfer(rewardPoolAddress, fee);
                _earnedAmt = _earnedAmt.sub(fee);
            }
        }
