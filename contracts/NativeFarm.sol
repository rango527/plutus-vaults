...
contract NativeFarm is Ownable, ReentrancyGuard {
    ...
    // Natives per block: (4.75)
    uint256 public NATIVEPerBlock = 4750000000000000000; // NATIVE tokens created per block, 50 million a year
    /// block number when vaults start
    uint256 public startBlock;
    /// block number when vaults end
    ...
    function updatePool(uint256 _pid) public {
        ...
        IBEP20(NATIVE).safeTransferFrom(stakingWallet, owner(), NATIVEReward.mul(ownerNATIVEReward).div(10000));
        IBEP20(NATIVE).safeTransferFrom(stakingWallet, address(this), NATIVEReward);
    ...
    function payReferralCommission(address _user, uint256 _pending) internal {
        if (address(nativeReferral) != address(0) && referralCommissionRate > 0) {
            ...
            if (referrer != address(0) && commissionAmount > 0) {
                IBEP20(NATIVE).safeTransferFrom(referrer, commissionAmount);
